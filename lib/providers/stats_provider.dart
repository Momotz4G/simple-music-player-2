...
class StatsNotifier extends StateNotifier<StatsState> {
  ...
  Future<Map<String, int>> _getAccurateHistoryCounts() async {
    try {
      ...
      final weekStartParsed = DateTime.parse(weekStartDay.replaceFirst(' ', 'T')).toLocal();
      ...
      DebugLogService().error("📊 Error calculating accurate history counts: $e");
  ...
  Future<void> syncNow() async {
    ...
    final automaticTitle = StatsUtils.getAutomaticTitle(calculated.totalMinutes);
    ...
    if (activeSelectedTitle == "Top 1 Global") {
      try {
        final topUsers = await PocketBaseService().fetchLeaderboard(sortBy: 'play_count', limit: 1, filter: 'nickname != ""');
        if (topUsers.isNotEmpty) {
          final topId = topUsers.first['user_id'];
          final myId = PocketBaseService().userId;
          if (topId != myId) {
            // ❌ LOST TOP 1! Revert immediately
            DebugLogService().warning("👑 LOST GLOBAL TOP 1! Reverting title...");
            Future.microtask(() => ref.read(profileProvider.notifier).updateTitle(null));
    ...
    await MetricsService().syncAdvancedStats(
      ...
      lastRepeatSongId: ref.read(sharedPrefsProvider).getString('last_played_track_id'),
      lastRepeatTime: ref.read(sharedPrefsProvider).getInt('last_track_play_time'),
  ...
  Future<void> _checkForHistoricalRewards() async {
    try {
      ...
      DebugLogService().info("🏆 [Hall of Fame] Detecting new week... checking last week's results.");
      ...
      final results = await PocketBaseService().fetchLeaderboard(
        ...
        limit: 10,
        filter: 'last_play_date >= "$lastWeekStart" && last_play_date < "$lastWeekEnd"',
      );
      ...
      if (myRank != -1 && myRank <= 3) {
        DebugLogService().success("🏆 CONGRATS! You finished Rank #$myRank last week!");
        ...
        DebugLogService().info("👟 [Hall of Fame] Finished Rank #$myRank last week. Better luck next time!");
      }
  ...
  Future<void> pullAndSyncRemoteData({bool isInitialLink = false}) async {
    try {
      ...
      final bool alreadyMigrated = prefs.getBool('has_migrated_stats_to_cloud') ?? false;
      ...
      if (isInitialLink && !alreadyMigrated) {
        ...
        final finalTotalPlays = (localCalculated.totalPlays > cloudPlayCount) ? localCalculated.totalPlays : cloudPlayCount;
        final finalDaily = (localCalculated.dailyPlays > cloudDaily) ? localCalculated.dailyPlays : cloudDaily;
        final finalWeekly = (localCalculated.weeklyPlays > cloudWeekly) ? localCalculated.weeklyPlays : cloudWeekly;
        finalMinutes = (localMinutes > cloudMinutes) ? localMinutes : cloudMinutes;
        ...
        DebugLogService().success("📊 STATS: Safe Max-Merge Migration Success. New Minutes: $finalMinutes, Total Plays: $finalTotalPlays");
      } else {
        // 🚀 SYNC/RESTORE MODE: Device has already migrated or is just syncing.
        if (cloudMinutes > localMinutes) {
          finalMinutes = cloudMinutes;
          DebugLogService().info("📊 STATS: Cloud higher ($cloudMinutes > $localMinutes). Restoring...");
        } else {
      ...
      if (finalMinutes > localMinutes) {
        // Note: For now we don't 'invent' missing play indices, 
        // we just ensure the aggregate totals are preserved.
        // A more advanced sync might fetch History records too.
      }
      ...
      // If anything changed, reload stats to refresh state
  ...
  Future<void> logPlay(SongModel song) async {
    ...
    MetricsService().trackSongPlayModel(
      song, 
    ...
    if (cloudSongId == id && cloudStreak > 0 && (nowMs - cloudTime <= 30 * 60 * 1000)) {
       if (lastTrackId != id || cloudStreak >= currentStreak) {
           currentStreak = cloudStreak;
           lastTrackId = id;
           lastTrackTime = nowMs; 
           DebugLogService().success("☁️ STREAK HANDSHAKE: Inherited streak of $currentStreak from cloud for $id");
       }
    }
    ...
    if (lastTrackId == id) {
      if (nowMs - lastTrackTime <= 30 * 60 * 1000) { 
        currentStreak++;
        DebugLogService().info("📈 STREAK: Consecutive play detected! Current: $currentStreak");
      } else {
        currentStreak = 1;
        DebugLogService().info("📈 STREAK: Same song but too much time passed (>30m). Resetting to 1.");
      }
    } else {
      currentStreak = 1;
      DebugLogService().info("📈 STREAK: New song detected. Resetting streak to 1.");
    }
    ...
    DebugLogService().info("📈 STREAK DEBUG: Song=${song.title}, Current=$currentStreak, Max=$maxStreak");
    DebugLogService().info("📈 STATS: Processed ${song.title}. Streak: $currentStreak, Max: $maxStreak");
  ...
  Future<void> repairLocalFromCloud() async {
    try {
      ...
      if (localMinutes > 0) {
        ...
        DebugLogService().info("📊 REPAIR: Applying ratio scaling ($ratio) to match cloud ($cloudMinutes mins)...");
        await applyGlobalAdjustment(ratio);
        DebugLogService().success("✅ REPAIR: Local stats adjusted successfully.");
